...
public interface NewsRepository extends JpaRepository<News, Long> {
    ...
    @Query("SELECT new com.tangtang.polingo.news.dto.NewsSummaryResponse(n.id, n.imageUrl, n.title, n.publishDate, (SELECT ns.originText FROM NewsSentence ns WHERE ns.news = n AND ns.id = (SELECT MIN(ns2.id) FROM NewsSentence ns2 WHERE ns2.news = n))) " +
            "FROM News n INNER JOIN n.newsScraps ns WHERE ns.user = :user AND (:language IS NULL OR n.language = :language) ORDER BY n.publishDate DESC")
    Page<NewsSummaryResponse> findScrapedNewsByUserAndLanguage(@Param("user") User user, @Param("language") Language language, Pageable pageable);
}
